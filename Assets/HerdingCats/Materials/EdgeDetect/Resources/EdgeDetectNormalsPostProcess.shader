...
Shader "Hidden/EdgeDetect-PostProcess"
{
	HLSLINCLUDE
		...
		float4 FragThin(VaryingsLum i) : SV_Target
		{
			...
			//return 0;
			//todo: THIS IS WHAT GOT EDITED
			//return edge * lerp(color, _BgColor, _BgFade);
			//color.a = 1;
			//float4 retcol = lerp(color, _BgColor, _BgFade) - ((1 - (edge)) * _Intensity);
			//float4 retcol = float4(color.rgb - ((1 - (edge)) * _Intensity), 1);
			//retcol.a = 1;
			return
				lerp(
					lerp(color, _BgColor, _BgFade), //Origin color
					float4(lerp(float3(1, 1, 1), float3(0, 0, 0), _Intensity), 1),
					ceil(1 -edge)
				)
				;
		}
